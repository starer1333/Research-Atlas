...
function renderResearch(){
  ...
  '<section class="section">'+sectionHead('BUSINESS MAP','这家公司靠什么赚钱','业务结构不与财务指标分离；分部是进一步研究 operating driver 的入口。')+'<div class="grid-2"><div class="paper">'+segments()+'</div><div class="paper"><span class="eyebrow">BUSINESS MODELS</span><h3 style="font:500 18px Georgia,serif">'+esc((v.business_map.business_models||[]).join(' / ')||'待补充')+'</h3><p style="font-size:12px;color:var(--muted);line-height:1.6">Industry: '+esc(v.business_map.industry||'unclassified')+'</p><span class="eyebrow" style="margin-top:18px">KNOWN GAPS</span><ul style="font-size:11px;color:var(--muted);line-height:1.7">'+(v.business_map.unknowns||[]).slice(0,4).map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul></div></div></section>'+

...
2026-09-18 后续补充：`scripts/probe_upstream.py` 已实际执行固定源码中的财务处理与估值模块，使用合成数据，不调用 API。确认零 EPS 被视为缺失、零 FCF 触发 EBITDA 比例替代、简化敏感区间不同于完整重算。详细数值、输入和取舍见 `v2-logic-and-validation.md`。这些发现服务于新产品设计，不等于完成上游整条报告链。产品计算引擎独立实现，目前没有复制上游模块代码。

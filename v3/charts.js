...
const colors=()=>({ink:css('--ink','#1d1d1f'),muted:css('--muted','#6e6e73'),line:css('--line','rgba(0,0,0,.10)'),accent:css('--accent','#0071e3'),accent2:css('--accent-2','#5e5ce6'),green:css('--green','#248a3d'),gold:css('--gold','#a65f00'),surface:css('--surface','#fff')});
...
function workingCapital(el,s){
 ...
 c.setOption({animationDuration:reduced()?0:600,legend:{top:0,left:0,textStyle:{color:p.muted,fontSize:10}},tooltip:{trigger:'axis',backgroundColor:'rgba(29,29,31,.92)',borderWidth:0,textStyle:{color:'#fff'}},grid:{left:50,right:18,top:42,bottom:38},xAxis:{type:'category',data:rows.map(x=>x.y),axisTick:{show:false},axisLine:{lineStyle:{color:p.line}},axisLabel:{color:p.muted,fontSize:9}},yAxis:{type:'value',splitLine:{lineStyle:{color:p.line}},axisLabel:{color:p.muted,fontSize:9,formatter:compact}},series:[['CFO','cfo',p.green],['Receivables','receivables',p.accent2],['Inventory','inventory',p.gold]].map(([name,k,color])=>({name,type:'line',data:rows.map(x=>x[k]??null),connectNulls:false,smooth:.18,symbolSize:6,lineStyle:{width:2.4,color},itemStyle:{color}}))},true)
}

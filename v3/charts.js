...
function workingCapital(el,s){
 ...
 c.setOption({animationDuration:reduced()?0:600,legend:{top:0,left:0,textStyle:{color:p.muted,fontSize:12}},tooltip:{trigger:'axis',backgroundColor:'rgba(29,29,31,.92)',borderWidth:0,textStyle:{color:'#fff'}},grid:{left:50,right:18,top:42,bottom:38},xAxis:{type:'category',data:rows.map(x=>x.y),axisTick:{show:false},axisLine:{lineStyle:{color:p.line}},axisLabel:{color:p.muted,fontSize:12}},yAxis:{type:'value',splitLine:{lineStyle:{color:p.line}},axisLabel:{color:p.muted,fontSize:12,formatter:compact}},series:[['CFO','cfo',p.dataCash],['Receivables','receivables',p.dataWorking],['Inventory','inventory',p.dataInventory]].map(([name,k,color])=>({name,type:'line',data:rows.map(x=>x[k]??null),connectNulls:false,smooth:.18,symbolSize:6,lineStyle:{width:2.4,color},itemStyle:{color}}))},true)
}

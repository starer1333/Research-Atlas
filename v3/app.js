...
function renderResearch(s){
 ...
 $('#content').innerHTML='<div class="hero"><div><span class="eyebrow">60-SECOND COMPANY VIEW</span><h1>'+esc(s.company.name)+'</h1><p>'+esc(s.company.subtitle||s.company.industry)+'。V3 先给你公司结构、财务轨迹和值得研究的问题；系统提出问题，但不会自动替用户形成最终判断。</p></div>'+coverage(s)+'</div>'+
 ...
 '<section class="section">'+head('V3-9 · FINANCIAL TRAJECTORY','先看趋势，再决定往哪里钻','ECharts 用于 inspection，不替代 evidence 和解释。')+'<div class="grid2"><div class="card chart-card"><div class="chart-title"><h3>Revenue × Operating Margin</h3><span>Dual-axis</span></div>'+chartHost('chart-trajectory','Revenue and operating margin trend')+'</div><div class="card chart-card"><div class="chart-title"><h3>Cash & Working Capital</h3><span>Relationship</span></div>'+chartHost('chart-working-capital','Cash and working capital trend')+'</div></div></section>'+
 '<section class="section">'+head('WHAT CHANGED','值得研究的变化','异常关系是 investigation trigger，不是“公司好/坏”的结论。')+'<div class="findings">'+(f.length?f.map(findingCard).join(''):'<div class="empty">没有触发预设高信号规则。</div>')+'</div></section>'+
 '<section class="section">'+head('QUESTIONS WORTH INVESTIGATING','从问题进入，而不是从模型参数进入','这是 V3 的主路径。')+f.map((x,i)=>'<div class="question"><span>0'+(i+1)+'</span><b>'+x.q+'</b><button class="linkbtn" data-q="'+i+'">开始研究 →</button></div>').join('')+'</section>'+
 '<section class="section">'+head('V3-10 · OPTIONAL INTELLIGENCE','AI 只规划研究，不拥有事实','Public preview 展示边界；真实 planner 仅在本地 runtime 显式启用。')+plannerPreview()+'</section>'
}

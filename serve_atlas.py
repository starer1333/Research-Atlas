...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--port',type=int,default=8766);parser.add_argument('--db',default=str(RUNTIME/'research.sqlite3'));parser.add_argument('--enable-sec',action='store_true');parser.add_argument('--sec-user-agent',default=os.environ.get('ATLAS_SEC_USER_AGENT',''));args=parser.parse_args()
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            try:
                if route=='/api/session':return self.reply({'token':token,'companies':store.companies(),'theme':store.theme(),'sources':{'sec':{'enabled':bool(sec_adapter),'adapter':'sec-edgar-direct','note':'Official SEC EDGAR; opt-in network access'}}})
